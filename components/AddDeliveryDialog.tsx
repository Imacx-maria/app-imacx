...
const AddDeliveryDialogInternal = ({
  ...
  const fetchPhcDataForFo = async (foNumber: string) => {
    ...
    try {
      ...
      const { data: phcFoData, error: phcFoError } = await supabase
        ...
        .select(
          "orcamento_number, customer_id, nome_trabalho, folha_obra_delivery_date, document_id",
        )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const localRecolha =
        armazens.find((a) => a.value === formData.id_local_recolha)?.label ||
        null;
      const localEntrega =
        armazens.find((a) => a.value === formData.id_local_entrega)?.label ||
        null;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-4 py-4">
          ...
          {phcItems.length > 0 && (
            <div className="space-y-2 imx-border-l bg-muted/30 py-3 pl-4 rounded">
              ...
              <div className="max-h-32 overflow-y-auto space-y-1 text-xs">
                {phcItems.map((item, idx) => (
                  <button
                    ...
                    className="w-full flex justify-between gap-2 py-2 px-2 imx-border-b hover:bg-accent rounded cursor-pointer transition-colors text-left"
                  >
                    <span className="flex-1 truncate">{item.description}</span>
                    <span className="text-muted-foreground">
                      Qt: {Math.round(Number(item.quantity))}
                    </span>
                  </button>
                ))}
              </div>
          ...
          {/* Transport Section */}
          <div className="space-y-4 imx-border-t pt-4">
