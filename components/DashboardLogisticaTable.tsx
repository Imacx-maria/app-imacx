...
> = ({
  ...
  // Pagination state
  const [currentPageEmCurso, setCurrentPageEmCurso] = useState(1);
  const [currentPageDespachados, setCurrentPageDespachados] = useState(1);
  ...
  const effectiveFilters = useMemo(
    () => ({
      guia:
        debouncedFilters.guia.trim().length >= 3 ? debouncedFilters.guia : "",
      numeroFo:
        debouncedFilters.numeroFo.trim().length >= 3
          ? debouncedFilters.numeroFo
          : "",
      numeroOrc:
        debouncedFilters.numeroOrc.trim().length >= 3
          ? debouncedFilters.numeroOrc
          : "",
      cliente:
        debouncedFilters.cliente.trim().length >= 3
          ? debouncedFilters.cliente
          : "",
      nomeCampanha:
        debouncedFilters.nomeCampanha.trim().length >= 3
          ? debouncedFilters.nomeCampanha
          : "",
      item:
        debouncedFilters.item.trim().length >= 3 ? debouncedFilters.item : "",
      codigo:
        debouncedFilters.codigo.trim().length >= 3
          ? debouncedFilters.codigo
          : "",
    }),
    [debouncedFilters],
  );
  ...
  const fetchData = useCallback(
    async (filterParams: Partial<typeof filters> = {}) => {
      // console.log(
      //   "📊 [DASHBOARD TABLE] Starting fetchData with filters:",
      //   filterParams,
      // );
      setLoading(true);
      try {
        ...
        const { data: logisticsData, error: logisticsError } =
          await logisticsQuery
            .order("created_at", { ascending: false })
            .limit(1000);

        if (logisticsError) {
          console.error(
            "❌ [DASHBOARD TABLE] Error fetching logistics:",
            logisticsError,
          );
          console.error(
            "❌ [DASHBOARD TABLE] Error details:",
            JSON.stringify(logisticsError, null, 2),
          );
          throw logisticsError;
        }

        // console.log(
        //   "✅ [DASHBOARD TABLE] Fetched logistics records:",
        ...
        // console.log(
        //   "✅ [DASHBOARD TABLE] Processed flat records:",
        //   flatRecords.length,
        // );

        setRecords(flatRecords);
        // Note: clientes, transportadoras, armazens are now passed as props from parent
        // and synced via useEffect - no need to fetch them here

        // console.log("✅ [DASHBOARD TABLE] Data fetch completed successfully");
      } catch (error) {
        console.error("💥 [DASHBOARD TABLE] Exception in fetchData:", error);
        console.error(
          "💥 [DASHBOARD TABLE] Error stack:",
          error instanceof Error ? error.stack : "No stack trace",
        );
        // Set empty records on error so loading completes
        setRecords([]);
      } finally {
        setLoading(false);
        // console.log(
        //   "🏁 [DASHBOARD TABLE] fetchData completed (loading set to false)",
        // );
      }
    },
    [supabase],
  ...
  // This fixes the issue where data doesn't load after fresh login
  useEffect(() => {
    let isMounted = true;
    ...
    const checkAuthAndFetch = async () => {
      try {
        ...
        if (isMounted) {
          setAuthReady(true); // Proceed anyway to show error state
        }
      }
    };
    ...
    // Listen for auth state changes (handles post-login scenario)
    const {
      data: { subscription },
  ...
  const filterRecords = useCallback(
    (recordsList: DashboardLogisticaRecord[]) => {
      return recordsList.filter((record) => {
        ...
        if (effectiveFilters.nomeCampanha?.trim()) {
          const campanhaFilter = effectiveFilters.nomeCampanha
            .trim()
            .toLowerCase();
  ...
  const createSorted = useCallback(
    (filtered: DashboardLogisticaRecord[]) => {
      if (!hasUserSorted) {
        return [...filtered];
      }
  ...
  const totalPagesEmCurso = Math.ceil(sortedEmCurso.length / ITEMS_PER_PAGE);
  const paginatedEmCurso = sortedEmCurso.slice(
    (currentPageEmCurso - 1) * ITEMS_PER_PAGE,
    currentPageEmCurso * ITEMS_PER_PAGE,
  );

  const totalPagesDespachados = Math.ceil(
    sortedDespachados.length / ITEMS_PER_PAGE,
  );
  const paginatedDespachados = sortedDespachados.slice(
    (currentPageDespachados - 1) * ITEMS_PER_PAGE,
    currentPageDespachados * ITEMS_PER_PAGE,
  );

  // Reset to page 1 when filters change
  useEffect(() => {
    setCurrentPageEmCurso(1);
  }, [debouncedFilters, dateFilter, selectedDate]);

  useEffect(() => {
    setCurrentPageDespachados(1);
  }, [debouncedFilters, dateFilter, selectedDate]);
  ...
  const handleConcluidoUpdate = useCallback(
    async (record: DashboardLogisticaRecord, value: boolean) => {
      try {
        if (!record.logistica_id) {
          console.error(
            "Cannot update concluido: missing logistica_id",
            record,
          );
          return;
        }
    ...
    <div className="w-full">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  size="sm"
                  onClick={onClearDate}
                  className="h-10 px-3 text-xs bg-primary hover:bg-primary/90 rounded-none"
                >
              ...
              <TooltipContent>
                Data selecionada no calendário - Clique para limpar
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
      ...
      <Tabs defaultValue="em_curso" className="w-full">
        <TabsList className="grid w-full grid-cols-2 rounded-none">
          <TabsTrigger value="em_curso" className="rounded-none">
            Em Curso ({sortedEmCurso.length})
          </TabsTrigger>
          <TabsTrigger value="despachados" className="rounded-none">
            Despachados ({sortedDespachados.length})
          </TabsTrigger>
        </TabsList>
            ...
            {totalPagesEmCurso > 1 && (
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-muted-foreground">
                  Página {currentPageEmCurso} de {totalPagesEmCurso} (
                  {sortedEmCurso.length} resultados)
                </div>
                <div className="flex gap-2">
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setCurrentPageEmCurso((p) => Math.max(1, p - 1))
                    }
                    disabled={currentPageEmCurso === 1}
                  >
                    Anterior
                  </Button>
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setCurrentPageEmCurso((p) =>
                        Math.min(totalPagesEmCurso, p + 1),
                      )
                    }
                    disabled={currentPageEmCurso === totalPagesEmCurso}
                  >
                    Próxima
                  </Button>
                </div>
        ...
        <TabsContent value="despachados" className="w-full">
          <div className="w-full">
            ...
            <Table className="w-full table-fixed uppercase imx-table-compact">
              <TableBody>
                {paginatedDespachados.length === 0 ? (
                  ...
                  paginatedDespachados.map((record) => (
                    <DashboardTableRow
                      key={`${record.item_id}-${record.logistica_id || "no-logistics"}`}
                      record={record}
                      editValues={editValues}
            ...
            {/* Pagination */}
            {totalPagesDespachados > 1 && (
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-muted-foreground">
                  Página {currentPageDespachados} de {totalPagesDespachados} (
                  {sortedDespachados.length} resultados)
                </div>
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() =>
                      setCurrentPageDespachados((p) => Math.max(1, p - 1))
                    }
                    disabled={currentPageDespachados === 1}
                  >
                    Anterior
                  </Button>
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setCurrentPageDespachados((p) =>
                        Math.min(totalPagesDespachados, p + 1),
                      )
                    }
                    disabled={currentPageDespachados === totalPagesDespachados}
                  >
                    Próxima
                  </Button>
                </div>
