-- =====================================================
-- GRANT PERMISSIONS FOR COMPANY CONVERSION RATES
-- =====================================================
-- Purpose: Fix "permission denied for table bo" error
-- Issue: get_company_conversion_rates needs access to phc schema tables
-- Solution: Grant EXECUTE with proper SECURITY DEFINER
-- =====================================================

-- Drop and recreate function with SECURITY DEFINER
-- This allows the function to run with the privileges of the owner (postgres/service_role)
...
SET search_path = public, phc  -- Explicitly set search path
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY
...
-- COMMENT
-- =====================================================
COMMENT ON FUNCTION get_company_conversion_rates IS
'Company-wide conversion rates by escalão with SECURITY DEFINER to access phc schema tables';
