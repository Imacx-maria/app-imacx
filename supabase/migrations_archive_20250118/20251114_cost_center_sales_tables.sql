-- ============================================================================
-- COST CENTER SALES TABLE FUNCTIONS (MTD + YTD)
-- ============================================================================
-- These functions power the dashboard tables. They intentionally mirror the
-- SQL shared by the finance team, only adding the cancellation/credit-note
-- filters they requested.
-- ============================================================================
...
sales_month AS (
  ...
  WHERE (
      (ft.document_type = 'Factura' AND ft.anulado = 'False')
...
sales_previous_year AS (
  ...
  WHERE (
      (ft.document_type = 'Factura' AND (ft.anulado IS NULL OR ft.anulado = 'False'))
      OR ft.document_type = 'Nota de Crédito'
    )
    AND fi.cost_center IN (
      'ID-Impressão Digital',
...
SELECT
  COALESCE(NULLIF(TRIM(COALESCE(sm.cost_center, py.cost_center)), ''), '(Sem Centro de Custo)') AS centro_custo,
...
SELECT
  COALESCE(
    NULLIF(TRIM(COALESCE(sy.cost_center, py.cost_center)), ''),
    '(Sem Centro de Custo)'
  ) AS centro_custo,
