-- Fix bo.budget_id to bo.document_number
-- The bo table uses document_number, not budget_id
...
COMMENT ON FUNCTION public.get_cost_center_top_customers IS
  'Returns top N customers per cost center for specified period (mtd or ytd) with invoice counts, quote counts (from bo.document_number where document_type=Orçamento), and conversion rates.';
