-- Fix ambiguous customer_id column reference in get_cost_center_top_customers
-- The RETURNS TABLE defines customer_id which conflicts with column selections
-- Solution: Use different internal column names and map them in final SELECT
...
COMMENT ON FUNCTION public.get_cost_center_top_customers IS
  'Returns top N customers per cost center for specified period (mtd or ytd). Fixed all ambiguous column references by using unique internal names.';
