-- =====================================================
-- DEPARTMENT ANALYSIS RPC FUNCTIONS
-- =====================================================
-- Purpose: Support Financial Analysis Departamentos tab
-- Uses proper user_siglas → profiles → departamentos join chain
-- Department names: "Brindes", "Digital", "IMACX"
-- =====================================================

-- =====================================================
-- 1. GET DEPARTMENT QUOTE BRACKETS (Escalões de Orçamentos)
-- =====================================================
-- Returns quote counts and values by value brackets for a department
...
BEGIN
  ...
  WITH department_quotes AS (
    -- Get all quotes for the department
    SELECT
      bo.document_id,
      bo.total_value
    FROM phc.bo bo
  ...
  bracketed_quotes AS (
    -- Assign each quote to a bracket
    SELECT
      CASE
        WHEN total_value < 1500 THEN '0-1500'
        WHEN total_value >= 1500 AND total_value < 2500 THEN '1500-2500'
        WHEN total_value >= 2500 AND total_value < 7500 THEN '2500-7500'
        WHEN total_value >= 7500 AND total_value < 15000 THEN '7500-15000'
        WHEN total_value >= 15000 AND total_value < 30000 THEN '15000-30000'
        ELSE '30000+'
      END as bracket,
      total_value
  ...
  total_value_sum AS (
    SELECT SUM(total_value) as grand_total FROM department_quotes
  )
...
-- =====================================================
-- 2. GET DEPARTMENT INVOICE BRACKETS (Escalões de Faturas)
-- =====================================================
-- Returns invoice counts and values by value brackets for a department
-- Same brackets as quotes
...
BEGIN
  ...
  WITH department_invoices AS (
    -- Get all invoices for the department (current year)
    SELECT
      ft.invoice_id,
      ft.net_value
    FROM phc.ft ft
    ...
    -- Get historical invoices (previous years)
    SELECT
      ft.invoice_id,
      ft.net_value
  ...
  bracketed_invoices AS (
    -- Assign each invoice to a bracket
    SELECT
      CASE
        WHEN net_value < 1500 THEN '0-1500'
        WHEN net_value >= 1500 AND net_value < 2500 THEN '1500-2500'
        WHEN net_value >= 2500 AND net_value < 7500 THEN '2500-7500'
        WHEN net_value >= 7500 AND net_value < 15000 THEN '7500-15000'
        WHEN net_value >= 15000 AND net_value < 30000 THEN '15000-30000'
        ELSE '30000+'
      END as bracket,
      net_value
  ...
  total_value_sum AS (
    SELECT SUM(net_value) as grand_total FROM department_invoices
  )
  -- Calculate percentages and return
  SELECT
    bs.bracket,
...
-- =====================================================
-- 3. GET DEPARTMENT CONVERSION RATES (Taxa de Conversão)
-- =====================================================
-- Returns conversion metrics by bracket: quotes vs invoices
-- Uses BiStamp chain to link quotes to invoices
...
BEGIN
  RETURN QUERY
  WITH department_quotes AS (
    -- Get all quotes for the department with bracket assignment
    SELECT
      bo.document_id,
      bo.total_value,
      CASE
  ...
  converted_quotes AS (
    -- Find quotes that were converted to invoices via BiStamp chain
    ...
    SELECT DISTINCT
      ...
      CASE
        ...
        ELSE '30000+'
      END as bracket,
      fi.net_value as invoiced_value
    FROM phc.bo bo
...
-- =====================================================
-- Returns customer counts: YTD, LYTD, new, lost
...
BEGIN
  ...
  WITH ytd_customers AS (
    -- Get unique customers YTD (current year invoices)
  ...
  lytd_customers AS (
    -- Get unique customers LYTD (historical invoices)
    SELECT DISTINCT ft.customer_id
    FROM phc."2years_ft" ft
    LEFT JOIN phc.cl cl ON ft.customer_id = cl.customer_id
    LEFT JOIN public.user_siglas us
...
-- =====================================================
-- Returns open quotes by age category for follow-up
-- Categories: Top 15 (biggest), Needs Attention (30-60 days), Lost (>60 days)
...
BEGIN
  RETURN QUERY
  WITH department_quotes AS (
    -- Get all open quotes for the department
    SELECT
      bo.document_number as quote_number,
    ...
    WHERE bo.document_date >= start_date
      ...
      AND bo.document_type = 'Orçamento'
      AND COALESCE(d.nome, 'IMACX') = departamento_nome
      -- Exclude converted quotes (those with invoices via BiStamp)
      AND NOT EXISTS (
        SELECT 1
        FROM phc.bi bi
        INNER JOIN phc.fi fi ON bi.line_id = fi.bistamp
  ...
  -- Return top 15 from each category
  SELECT
    quote_number,
    quote_date,
    customer_name,
    ROUND(quote_value, 2) as quote_value,
    status,
    days_open,
    category
