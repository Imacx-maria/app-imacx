-- Fix ambiguous column reference in get_cost_center_top_customers
-- The issue was SUM(invoice_count) in center_stats CTE needed full qualification
...
COMMENT ON FUNCTION public.get_cost_center_top_customers IS
  'Returns top N customers per cost center for specified period (mtd or ytd). Uses dynamic date calculations based on CURRENT_DATE. Fixed ambiguous column reference.';
