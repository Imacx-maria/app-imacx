"""
Runner script for fast BO/BI ETL using watermarks

Steps:
- Imports SelectiveSync from scripts/etl_core/selective_sync.py
- Executes sync_fast_bo_bi_watermarked()
...
try:
    ...
    PROJECT_ROOT = THIS_FILE.parents[2]  # scripts/etl/run_fast_bo_bi_sync.py -> project root
...
def main() -> int:
    try:
        sync = SelectiveSync()
        results = sync.sync_fast_bo_bi_watermarked()
